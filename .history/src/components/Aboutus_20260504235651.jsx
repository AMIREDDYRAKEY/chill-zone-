    ...
    return (
        <div>
            ...
            <div className='flex  p-10 flex-col gap-5'>
                {data.map((data,i)=>(
                    <div key={i} className='border-[1px] items-center gap-5 p-4 w-[600px] h-[100px] flex '>
                        <div className='text-[20px] px-3 py-3 border-[1px] bg-[#0f2436] rounded-xl border-[#0d4157] '>{data.logo}</div>
                        <div>
                        <h1>{data.name}</h1>
                        <p>{data.des}</p>
                        </div>
